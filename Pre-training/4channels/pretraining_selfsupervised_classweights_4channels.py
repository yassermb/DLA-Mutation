...
def Conv_3D_model(input_shape):
    ...
    H = Conv3D(4, kernel_size=(3, 3, 3), use_bias = True, padding = 'valid', activation='elu', kernel_initializer='he_uniform', input_shape=X_in.shape)(X_in)
    H = BatchNormalization()(H)
    #H = MaxPooling3D(pool_size=(2, 2, 2))(H)
    H = Conv3D(4, kernel_size=(4, 4, 4), use_bias = True, padding = 'valid', activation='elu', kernel_initializer='he_uniform', input_shape=H.shape)(H)
    H = BatchNormalization()(H)
    #H = MaxPooling3D(pool_size=(2, 2, 2))(H)
    H = Conv3D(4, kernel_size=(4, 4, 4), use_bias = True, padding = 'valid', activation='elu', kernel_initializer='he_uniform', input_shape=H.shape)(H)
    H = BatchNormalization()(H)
...
for foldk in range(5):
    ...
    input_shape=(v_dim,v_dim,v_dim,4)
    model = Conv_3D_model(input_shape)
    #model = load_model('0_999_model')
