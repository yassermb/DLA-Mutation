...
#df['gemme_jet'] = df.complex.str.replace('mut_','').str.replace('wt_','').str[:-4].apply(lambda x: list(gemme_jet_dict[x]) if x in gemme_jet_dict else [0,0,0,0,0])
df['gemme_jet'] = df.complex.apply(lambda x: list(gemme_jet_dict['--'.join(x.split('--')[:-1]).replace('mut_','').replace('wt_','')]) if x in gemme_jet_dict else [0,0,0,0,0])
...
for seq_sim in ['30', '50', '70', '90', '95', '100']:
    df['ht'+str(seq_sim)] = df.complex.str.replace('mut_','').str.replace('wt_','').str[:4].apply(lambda x: ','.join(map(lambda xx: str(xx), skempi_homology[seq_sim][x])))
...
df_train = df.loc[df[separation_level].isin(clusters_set[:int(0.6*len(clusters_set))])]
df_test = df.loc[df[separation_level].isin(clusters_set[int(0.4*len(clusters_set)):])]




df_train.complex.to_csv('transfer_learning_aa_train_complexes.csv', sep=',')
df_test.complex.to_csv('transfer_learning_aa_test_complexes.csv', sep=',')

#For training remove backrub mutations and only keep backrub WT
df_train = df_train.loc[~df_train.complex.str.contains('mut_')]
...
#For testing divide the test set into two parts: wt_backrub and mut_backrub
df_test_wt = df_test.loc[df_test.complex.str.contains('wt_')]
df_test_mut = df_test.loc[df_test.complex.str.contains('mut_')]
...
#X_train_aux = df_train.auxiliary.to_numpy()
...
#X_test_aux = df_test.auxiliary.to_numpy()
...
X_test_aux_wt = []
y_test_wt = []
instances = list(set(map(lambda x: '--'.join(x.split('--')[:-1]), df_test_wt.complex.to_list()))) #just removing the backrub model code!
for instance in instances:
    X_test_wt.append(df_test_wt.loc[df_test_wt.complex.str.contains(instance)][embedding_columns].astype(np.float32).mean().to_numpy())
    X_test_aux_wt.append(df_test_wt.loc[df_test_wt.complex.str.contains(instance)].iloc[0].auxiliary.astype(np.float32))
    y_test_wt.append(df_test_wt.loc[df_test_wt.complex.str.contains(instance)].iloc[0].target.astype(np.float32))
X_test_wt = np.array(X_test_wt)
X_test_aux_wt = np.array(X_test_aux_wt)
y_test_wt = np.array(y_test_wt)
...
X_test_aux_mut = []
y_test_mut = []
instances = list(set(map(lambda x: '--'.join(x.split('--')[:-1]), df_test_mut.complex.to_list()))) #just removing the backrub model code!
for instance in instances:
    X_test_mut.append(df_test_mut.loc[df_test_mut.complex.str.contains(instance)][embedding_columns].astype(np.float32).mean().to_numpy())
    X_test_aux_mut.append(df_test_mut.loc[df_test_mut.complex.str.contains(instance)].iloc[0].auxiliary.astype(np.float32))
    y_test_mut.append(df_test_mut.loc[df_test_mut.complex.str.contains(instance)].iloc[0].target.astype(np.float32))
X_test_mut = np.array(X_test_mut)
X_test_aux_mut = np.array(X_test_aux_mut)
y_test_mut = np.array(y_test_mut)
###############################################################################################
...
print(y_test_wt.sum(axis=0))
print(y_test_mut.sum(axis=0))


train_input = [X_train, X_train_aux]
test_input_wt = [X_test_wt, X_test_aux_wt]
test_input_mut = [X_test_mut, X_test_aux_mut]


#model = get_model(200, 10)
model = get_model(200, 5)
history = model.fit(train_input, y_train, validation_data=(test_input_wt, y_test_wt), batch_size=50, epochs=200, verbose=1, class_weight=d_class_weights)
...
test_preds_wt = model.predict(test_input_wt)
test_preds_mut = model.predict(test_input_mut)
"""
#######################################################
#Averaging embeddings over backrub models
test_preds = []
...
def performance(y_test, test_preds, name):
    ...
    np.save(output_matrix+'_'+name, cf_matrix)
    ...
    plt.tight_layout()
    plt.savefig(output_confusionmatrix_svg.replace('.svg', '_'+name+'.svg'))
    ...
    plt.ylim(0,1)
    plt.tight_layout()
    plt.savefig(output_precisionrecall_svg.replace('.svg', '_'+name+'.svg'))
    ...
    plt.ylim(0,1)
    plt.tight_layout()
    plt.savefig(output_f1_svg.replace('.svg', '_'+name+'.svg'))
