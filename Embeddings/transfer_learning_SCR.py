...
df = pd.read_csv('intermediate_backrub_nomask_200.csv', sep='\t')
...
#Add GEMME and JET2 features
gemme_jet_dict = load_obj('comp_mut_map_gemme_jet')
df['gemme_jet'] = df.complex.apply(lambda x: np.array(gemme_jet_dict['--'.join(x.split('--')[:-1]).replace('mut_','').replace('wt_','')]) if x in gemme_jet_dict else np.array([0,0,0,0,0]))
#.str.replace('mut_','').str.replace('wt_','').str[:-4]
...
X_train_aux = np.array(list(map(lambda x: x.astype(np.float32), df_train['gemme_jet'].to_numpy())))
...
X_test_aux = np.array(list(map(lambda x: x.astype(np.float32), df_test['gemme_jet'].to_numpy())))
...
for instance in instances:
    ...
    X_test_aux.append(df_test.loc[df_test.complex.str.contains(instance)].iloc[0].gemme_jet.astype(np.float32))
